...
from car_cost.models import InputFormModel, EngineType
...
class CarCostApp:
    _tax_computer: TaxComputer
    _loan_computer: LoanComputer
    ...
    def __init__(
        ...
        self._tax_computer = tax_computer
        self._loan_computer = loan_computer
    ...
    def _get_tax_deduction(
        ...
        new_tax = self._tax_computer.compute_tax(
        ...
        max_tax = self._tax_computer.compute_tax(
            taxable_revenue=taxable_revenue,
            work_distance=0,
            administrative_power=administrative_power,
            engine_type=engine_type,
    ...
    def _get_costs(self, data: InputFormModel) -> pd.DataFrame:
        ...
        ] += self._loan_computer.compute_annuity(
    ...
    def _get_debts(self, data: InputFormModel) -> pd.DataFrame:
        ...
        for year in range(1, data.settings.n_years + 1):
            debt = self._loan_computer.compute_debt(
                year=year,
                capital=data.financing.loan,
                rate_percentage=data.financing.loan_rate,
                duration=data.financing.loan_duration,
    ...
    def run(self) -> None:
        ...
        data = sp.pydantic_form(
            key="input_form", model=InputFormModel, submit_label="Soumettre"
        )
